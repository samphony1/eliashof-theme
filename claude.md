...
- **Created Modular Section Snippets:** Replaced the master "page-unsere-schule" pattern with three separate modular block patterns: `patterns/section-textbox.php` (transparent textbox below hero), `patterns/section-ueber-uns.php` (solid green grid matching SPB layout), and `patterns/section-leitbild.php` (transparent Leitbild block).
