...
- **Eltern Hero Pattern & Illustration 04**: Created the custom `Eltern Hero` block pattern (`patterns/hero-blog-eltern.php`, slug `eliashof/hero-blog-eltern`) using the translucent yellow multiply overlay (`.bg-graph-yellow`) and `illustration04.svg` image asset.
