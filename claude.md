...
- **Created Modular Section Snippets:** Replaced the master "page-unsere-schule" pattern with four separate modular block patterns: `patterns/section-textbox.php` (transparent textbox below hero), `patterns/section-ueber-uns.php` (solid green grid matching SPB layout), `patterns/section-leitbild.php` (transparent Leitbild block), and `patterns/section-parallax.php` (full-width parallax image section).
