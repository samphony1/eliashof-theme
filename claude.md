...
- **Created Modular Section Snippets:** Replaced the master "page-unsere-schule" pattern with five separate modular block patterns: `patterns/section-textbox.php` (transparent textbox below hero), `patterns/section-ueber-uns.php` (solid green grid matching SPB layout), `patterns/section-leitbild.php` (transparent Leitbild block), `patterns/section-parallax.php` (full-width parallax image section), and `patterns/section-ausstattung.php` (transparent Ausstattung block).
...
- **Synchronized Textblock Paddings & Widths:** Configured identical horizontal padding (`clamp(24px, 10.8vw, 208px)`) and maximum container width (`1497px`) for all modular textblocks (`.eliashof-textbox-section`, `.eliashof-leitbild-section`, and `.eliashof-ausstattung-section`) to align perfectly with the SPB layout's text columns.
- **Preconfigured Block HTML Anchors:** Added default HTML anchor definitions (`id="..."` and Gutenberg `"anchor"` block attributes) on the parent container blocks of all 14 custom Eliashof block patterns to support direct hash linking in menus (e.g. `#ueber-uns`, `#foerderverein`, `#section-ausstattung`, etc.) even when layout editing is locked.
