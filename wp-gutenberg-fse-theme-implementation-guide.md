...
- **Translucent Yellow Hero Background:** Converted `.bg-graph-yellow` back to use a translucent multiply-blend overlay (`background-color: transparent !important`, `mix-blend-mode: multiply`, using color `#F6D59F`), allowing the underlying paper grid pattern to shine through.
- **School/Blog Hero Image Proportions:** Resized the columns of `.eliashof-hero-blog-schule` to match Figma specifications: set the image column to `43.05%`, content column to `51.95%`, and set the image container to the exact aspect ratio `828.67 / 546.75`. Added responsive scaling to make them scale proportionally and prevent overlap.
...
- **Body Background Margin Shifts**: Added media query-based background shifts for the body paper grid pattern: shifts the image left by `-170px` on tablet viewports (`768px` to `991px`) to keep the red vertical margin line at a clear `35px` from the left edge (inside the page margins), and shifts by `-260px` on mobile viewports (< `768px`) to push it off-screen, completely avoiding any overlapping readability conflict.
- **Eltern Hero Pattern & Illustration 04**: Created the custom `Eltern Hero` block pattern (`patterns/hero-blog-eltern.php`, slug `eliashof/hero-blog-eltern`) using the translucent yellow multiply overlay (`.bg-graph-yellow`), `illustration04.svg` image asset, and an editable heading block.
- **Unsere Schule Page Hero Pattern**: Created a new `Unsere Schule - Seite` block pattern (`patterns/hero-blog-schule-page.php`, slug `eliashof/hero-blog-schule-page`) with a static editable heading block so custom page titles can be manually overwritten, while preserving the original dynamic `hero-blog-schule` pattern for single blog post views (`single.html`).
- **Hero Title Top Alignment**: Set the title position in both `Schule Hero` and `Eltern Hero` to align to the top (`justify-content: flex-start !important;`) with `0` top padding, and reduced their maximum dynamic font-size to `60px` (`clamp(32px, 3.8vw, 60px)`) to optimize spacing and readability.
- **Nested Wrapper Migration (Page 106)**: Added automatic database cleanup migration in `functions.php` to remove the nested duplicate `.bg-graph-blue` wrapper blocks from the "Unsere Schule" page (ID 106), resolving the double-opacity blue background issue. Fixed the trigger so that it only runs if the nested outer wrapper comments are present, preserving any manual text edits to the heading block by the user.
- **Illustration Right Alignment**: Updated subpage hero illustration container styles to align the graphics all the way to the right (`right: 0 !important;`) of the content container.
- **Aktuelles (Eltern) Pattern**: Created a new `Aktuelles (Eltern)` news carousel block pattern (`patterns/aktuelles-eltern.php`, slug `eliashof/aktuelles-eltern`) filtering posts by the dynamically looked up `eltern` category term ID.
- **GEV / Links Section Centering**: Added CSS style overrides to center text inside GEV and Links flex columns on the Eltern page.
- **Global Orange Text Links**: Added styling rules to color all standard body/post text links on the Eliashof site orange (`#B55A00`) and display them with an underline text decoration transition on hover. Set this color in both CSS and `theme.json` to prevent WordPress accessibility and contrast check warnings on white/cream backgrounds.
- **Eltern Infos Pattern**: Created a standalone `Eltern Infos` block pattern (`patterns/section-eltern-infos.php`, slug `eliashof/section-eltern-infos`) with centered columns styled to stretch to the exact same height (`align-items: stretch !important;`).
- **Eltern Page Hero & GEV Migrations (Page 209)**: Executed database cleanup migration to consolidate stacked duplicate hero sections into a single correct-colored hero, and replaced raw GEV/Links column block markup with the new `eliashof/section-eltern-infos` block pattern.
- **GEV (Sektion) Pattern & Reversed Column Layout**: Created the custom `GEV (Sektion)` block pattern (`patterns/section-gev.php`, slug `eliashof/section-gev`) duplicating the SPB structure (centered title, columns below) but with swapped columns (text left, image right). Configured matching layout, font and responsive media query styles for `.section-gev` in `style.css`.
- **Aktuelles Archiv Page Pattern**: Created the custom page block pattern (`patterns/page-aktuelles-archiv.php`, slug `eliashof/page-aktuelles-archiv`) with a blue subpage hero and transparent post grid container.
- **Dynamic Category Filters Shortcode**: Registered the `[eliashof_aktuelles_archiv]` shortcode in `functions.php` rendering a responsive 3-column posts grid and centered category buttons using standard button styles. Features a vanilla JS transition that filters categories in real-time.
- **Aktuelles Page Setup & Menu Link updates**: Created and initialized the page (ID 251, slug `/aktuelles/`) with featured image 198 (school yard). Updated primary navigation menu item ID 97 and fallback menu callback to point to the new page instead of the anchor `/#aktuelles`.
- **Transparent Post Grid Background**: Added a `.bg-transparent` utility style in `style.css` and applied it to the Aktuelles post grid container, ensuring the site's default body paper grid texture background shines through.
- **Round Frame Post Card Images**: Styled Aktuelles grid post card images with matching custom aspect ratio (`684/471.6` or `1.45`) and rounded corners (`border-radius: 40px`), mirroring the big image sections across the rest of the site.
